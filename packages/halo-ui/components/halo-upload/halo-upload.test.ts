...
import HaloUpload from './halo-upload.vue'

describe('HaloUpload cancellation', () => {
  it('aborts an injected uploader when an active upload is cancelled', async () => {
    ...
    const wrapper = mount(HaloUpload, {
      props: {
    ...
    await wrapper.find('.halo-upload__trigger').trigger('tap')
    await wrapper.vm.$nextTick()
    await new Promise((resolve) => setTimeout(resolve, 0))
    const active = wrapper.find('.halo-upload__item--uploading')
