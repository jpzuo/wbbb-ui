export interface WbbbSelectionOption {
  value: string | number
}
 ...
 * conflating `1` and `'1'`.
 */
export function getOptionByIndex<T extends WbbbSelectionOption>(options: T[], rawIndex: string | number): T | undefined {
  const index = Number(rawIndex)

  return Number.isInteger(index) && index >= 0 ? options[index] : undefined
}

export function getOptionsByIndex<T extends WbbbSelectionOption>(options: T[], rawIndexes: Array<string | number>): T[] {
  return rawIndexes
