<template>
  <view v-if="visible" class="wbbb-notify" :class="[customClass, `wbbb-notify--${type}`]" :style="rootStyle">
    <text class="wbbb-notify__message"><slot>{{ message }}</slot></text>
  </view>
</template>
...
import type { WbbbNotifyProps } from './props'

const props = withDefaults(defineProps<WbbbNotifyProps>(), {
  customClass: '',
  customStyle: '',
  duration: 2200,
  message: '',
